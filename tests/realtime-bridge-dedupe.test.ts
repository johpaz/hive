 ...
 * tienes en el espacio de trabajo?" terminó en TRES tareas idénticas delegadas a
...
describe("similitud entre pedidos de voz", () => {
  ...
  test("reconoce el mismo pedido reformulado por el modelo", () => {
    const pares: Array<[string, string]> = [
      [
        ...
        "Lista los archivos que hay en el espacio de trabajo.",
      ],
      [
      ...
      [
        "Busca en la web qué es el protocolo MCP y resúmelo.",
  ...
  test("no confunde pedidos distintos", () => {
    const pares: Array<[string, string]> = [
      ...
      [
        "Lee el contenido del archivo resumen_tecnico_hive_quiz.docx y resúmelo.",
        "Programa un recordatorio para mañana a las nueve de la mañana.",
      ],
      [
        "Busca el precio del dólar hoy.",
        "Escribe un informe gerencial sobre la reunión de ayer.",
      ],
    ];
  ...
  test("corta la ráfaga de reintentos del modelo", () => {
    ...
    const reintento = "Lista los archivos que hay en el espacio de trabajo.";
  ...
  test("un pedido distinto pasa aunque comparta palabras", () => {
    expect(
      esDuplicado("El usuario pide que liste los archivos del espacio de trabajo.",
                  "Busca en la web el precio del dólar hoy.", EN_CALIENTE),
    ).toBe(false);
  });
  ...
  test("pasado el rato, un pedido parecido vuelve a valer", () => {
    ...
    expect(esDuplicado("Busca el precio del dólar hoy.", "Busca el precio del euro hoy.", MAS_TARDE))
      .toBe(false);
    expect(esDuplicado("Lee el archivo informe.docx", "Lee el archivo resumen.docx", MAS_TARDE))
      .toBe(false);
  });
  ...
  test("ignora tildes, mayúsculas y puntuación", () => {
    ...
    expect(similitud("Lista los archivos del espacio", "lista LOS ARCHIVOS del espacio!!")).toBe(1);
    expect(similitud("¿Qué versión tienes instalada?", "que version tienes instalada")).toBe(1);
  });
