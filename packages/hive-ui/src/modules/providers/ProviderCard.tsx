...
import { Settings, ChevronDown, ChevronRight, Cpu, Plus, KeyRound, Globe, RefreshCw } from "lucide-react";
...
export function ProviderCard({ provider, updateProvider, onManageModels }: ProviderCardProps) {
  ...
  const { models, toggleModel, createModel, syncModels } = useModels();
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between px-2.5 py-1.5 rounded-lg transition-colors ${
                      modelActive ? 'bg-white/5' : 'bg-transparent opacity-40'
                    }`}
                  >
                    <span className="text-[11px] font-medium text-white truncate flex-1 mr-3">
                      {model.name}
                    </span>
                    <Switch
                      checked={modelActive}
                      onCheckedChange={(checked) => handleModelToggle(model, checked)}
                      className="scale-[0.7] origin-right data-[state=checked]:bg-blue-400"
                    />
                  </div>
                );
              })}
