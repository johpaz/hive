...
/**
 * Cómo se llama la voz. Fijo, no configurable.
 *
 * Antes tomaba el nombre del coordinador que hubiera en la base de datos, y la
 * misma figura se presentaba con un nombre distinto en cada instalación
 * mientras la escena, la documentación y la interfaz seguían diciendo BIA. El
 * nombre del coordinador es de la colmena y se ve en el chat; éste es el de la
 * voz que habla.
 */
export const VOICE_PERSONA = "BIA";
...
export interface VoicePromptInput {
  /** Nombre de quien habla, para dirigirse a esa persona (users.name). */
...
export function buildVoicePrompt(input: VoicePromptInput): string {
  ...
  return [
    ...
    "- Los mensajes [HIVE] solo te traen los hitos. El detalle lo tienes en \"estado_de_la_colmena\": qué hace",
    "  Bee, qué especialista tiene cada tarea y con qué herramienta, qué terminó y con qué resultado o error, y",
    "  los últimos pasos de esta conversación. Responde en el acto: úsala sin miedo.",
    "- Si la persona pregunta qué está pasando, quién trabaja, cómo va, por qué tarda o qué salió, consúltala",
    "  antes de responder. Nunca contestes eso de memoria ni lo supongas.",
    "- Cuéntalo con tus palabras y en pocas frases: quién está en qué, qué ya terminó y qué falta.",
