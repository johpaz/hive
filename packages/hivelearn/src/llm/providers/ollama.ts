...
export class OllamaProvider implements LLMProvider {
  async call(options: LLMCallOptions): Promise<LLMResponse> {
    ...
    const client = new Ollama({ host, ...(Object.keys(headers).length ? { headers } : {}) })
    ...
    const runtimeOptions: Record<string, unknown> = { num_ctx: options.numCtx ?? 4096 }
    ...
    try {
      log.info(
        ...
        ` — ${messages.length} msgs, ${tools?.length ?? 0} tools num_ctx=${runtimeOptions.num_ctx}`
      )
      ...
      let content = ''; let promptEvalCount = 0; let evalCount = 0
      ...
      return {
        ...
        usage: evalCount > 0 ? { input_tokens: promptEvalCount, output_tokens: evalCount } : undefined,
      }
    } catch (error: any) {
      log.error(`[llm-client] FAILED call to ollama/${modelName} at ${host}`)
      log.error(`[llm-client] Error details: ${error.message || error}`)

      if (error.message?.includes('model runner has unexpectedly stopped') && tools?.length) {
        log.warn(`[llm-client] OOM with tools — retrying without tools`)
        const stream2 = await client.chat({ model: modelName, messages, tools: undefined, options: runtimeOptions, stream: true })
        let content = ''
