      ...
      <div className="w-full max-w-lg">
        ...
        <div className="mb-6">
          <div className="flex justify-between text-xs text-gray-500 mb-1">
            ...
            <span className="text-amber-400">{Math.round(((preguntaActual) / total) * 100)}%</span>
          </div>
        ...
        <div className="bg-gray-900 rounded-2xl border border-gray-800 p-6 space-y-4">
          <div className="flex items-start gap-2">
            <span className="shrink-0 rounded-full bg-amber-500/20 border border-amber-500 w-6 h-6 flex items-center justify-center text-xs font-bold text-amber-400">
              {preguntaActual + 1}
            </span>
            ...
            <div className="space-y-2">
              {pregunta.opciones.map((op, i) => (
                <button
                  ...
                  className="w-full text-left rounded-xl border border-gray-700 bg-gray-800 px-4 py-3 text-sm text-gray-300 hover:border-amber-500 hover:text-white transition-all"
                >
                  <span className="font-bold text-amber-400 mr-2">{String.fromCharCode(65 + i)}.</span>
                  {op}
                </button>
              ))}
            </div>
          ...
          {pregunta.tipo === 'respuesta_corta' && (
            <div className="space-y-3">
              <textarea
                className="w-full rounded-xl bg-gray-800 border border-gray-700 px-4 py-3 text-sm text-white focus:border-amber-500 outline-none resize-none h-24"
              ...
              <button
                ...
                className="w-full rounded-xl bg-amber-500 py-3 text-sm font-bold text-black hover:bg-amber-400 disabled:opacity-40 disabled:cursor-not-allowed"
              >
